...
# Run with --help to see all options and their current defaults.
#
# The version constants below are the single source of truth. The runtime
# help text, the generated README examples, and scripts/test-phase2-on-riscv64.sh
# all derive their versions from these values, so a bump here propagates
# everywhere with no manual edits.
...
Options:
  --docker-version VERSION     Docker Engine version (default: ${DOCKER_VERSION})
  --cli-version VERSION        Docker CLI version (default: ${CLI_VERSION})
  --compose-version VERSION    Docker Compose version (default: ${COMPOSE_VERSION})
  --containerd-version VERSION Containerd version (default: ${CONTAINERD_VERSION})
  --runc-version VERSION       Runc version (default: ${RUNC_VERSION})
  --tini-version VERSION       Tini version (default: ${TINI_VERSION})
...
emerge -av =app-containers/containerd-@CONTAINERD_VERSION@

# Update only Docker CLI
emerge -av =app-containers/docker-cli-@CLI_VERSION@

# Update Docker Engine (will check dependencies)
emerge -av =app-containers/docker-@DOCKER_VERSION@
```
...
emerge -C =app-containers/docker-@DOCKER_VERSION@-r1
...
# Substitute version placeholders in the generated README so its examples
# always match the constants above (single source of truth). The heredoc is
# quoted to keep markdown backticks literal, so versions go in as @VAR@ tokens
...
sed -i \
    ...
    -e "s/@RUNC_VERSION@/${RUNC_VERSION}/g" \
